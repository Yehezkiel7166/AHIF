...
steps=(verify-config validate regression runtime empirical framework-audit failure-injection health release-check python-syntax shell-syntax)
...
run empirical python3 -m unittest discover -s 14_TESTS/empirical_validation -p 'test_*.py' -v
...
run python-syntax python3 -m compileall -q scripts RUNTIME 22_EMPIRICAL_VALIDATION 14_TESTS/runtime 14_TESTS/empirical_validation
